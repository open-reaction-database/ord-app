...
if command -v clang-format-10 &> /dev/null; then
  find "${ROOT_DIR}" -name '*.js' ! -path '*/ketcher/*' ! -path '*/node_modules/*' -exec clang-format-10 --verbose -i {} +
elif command -v clang-format &> /dev/null; then
  # NOTE(kearnes): Make sure you have version 10 or higher!
  find "${ROOT_DIR}" -name '*.js' ! -path '*/ketcher/*' ! -path '*/node_modules/*' -exec clang-format --verbose -i {} +
else
