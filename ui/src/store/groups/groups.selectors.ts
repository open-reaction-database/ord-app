...
export const selectMemberRoles = createSelector(
  ...
  (editingGroupId, state, currentUser) => {
    const groupMembers = editingGroupId !== null ? selectGroupMembersByGroupId(editingGroupId)(state) : null;
    const isAdmin = groupMembers?.find(member => member.user.id === currentUser?.id)?.role === USER_ROLES.ADMIN;
    const admins = groupMembers?.filter(member => member.role === USER_ROLES.ADMIN);
    const hasTwoAdmins = admins && admins.length >= 2;
